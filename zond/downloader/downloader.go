...
func (d *Downloader) fetchBodies(from uint64) error {
	...
	err := d.concurrentFetch((*bodyQueue)(d), true)
...
func (d *Downloader) processHeaders(origin uint64) error {
	...
	defer func() {
		if rollback > 0 {
			lastHeader, lastFastBlock, lastBlock := d.blockchain.CurrentBlock().Number, d.blockchain.CurrentSnapBlock().Number, d.blockchain.CurrentBlock().Number
			...
			log.Warn("Rolled back chain segment",
				"header", fmt.Sprintf("%d->%d", lastHeader, d.blockchain.CurrentBlock().Number),
