	...
	tests := []struct {
		...
		// Mainnet test cases
		{
			...
			[]testcase{
				...
				// NOTE(rgeraldes24): no forks available atm
				/*
...
func TestValidation(t *testing.T) {
	...
	tests := []struct {
		...
		{&legacyConfig, 15050000, 0, ID{Hash: checksumToBytes(0x6170f487), Next: 0}, nil},

		// Local is mainnet Shanghai, remote announces the same. Remote also announces a next fork
		// at block 0xffffffff, but that is uncertain.
		{&legacyConfig, 15050000, 0, ID{Hash: checksumToBytes(0x6170f487), Next: math.MaxUint64}, nil},
...
func TestTimeBasedForkInGenesis(t *testing.T) {
	...
	tests := []struct {
		...
		// NOTE(rgeraldes24): no forks available for now
		/*
