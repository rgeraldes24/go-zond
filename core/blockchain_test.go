...
func testLastBlock(t *testing.T, scheme string) {
	...
	blocks := makeBlockChain(blockchain.chainConfig, blockchain.GetBlockByHash(blockchain.CurrentBlock().Hash()), 1, beacon.New(), genDb, 0)
...
func testEIP155Transition(t *testing.T, scheme string) {
	...
	genDb, blocks, _ := GenerateChainWithGenesis(gspec, beacon.NewFaker(), 4, func(i int, block *BlockGen) {
		...
		switch i {
		case 0:
			tx, err = basicTx(types.NewShanghaiSigner(common.Big1))
			if err != nil {
				t.Fatal(err)
			}
			block.AddTx(tx)
	...
	// TODO(rgeraldes24): this part is not valid anymore
...
func testDeleteCreateRevert(t *testing.T, scheme string) {
	...
	_, blocks, _ := GenerateChainWithGenesis(gspec, engine, 1, func(i int, b *BlockGen) {
		...
		tx, _ := types.SignTx(types.NewTransaction(0, aa,
			big.NewInt(0), 50000, b.header.BaseFee, nil), types.NewShanghaiSigner(common.Big1), key)
		b.AddTx(tx)
		// One transaction to BBBB
		tx, _ = types.SignTx(types.NewTransaction(1, bb,
			big.NewInt(0), 100000, b.header.BaseFee, nil), types.NewShanghaiSigner(common.Big1), key)
		b.AddTx(tx)
	})
	// Import the canonical chain
...
func testDeleteRecreateAccount(t *testing.T, scheme string) {
	...
	_, blocks, _ := GenerateChainWithGenesis(gspec, engine, 1, func(i int, b *BlockGen) {
		...
		tx, _ = types.SignTx(types.NewTransaction(1, aa,
			big.NewInt(1), 100000, b.header.BaseFee, nil), types.NewShanghaiSigner(common.Big1), key)
		b.AddTx(tx)
	})
	// Import the canonical chain
	chain, err := NewBlockChain(rawdb.NewMemoryDatabase(), DefaultCacheConfigWithScheme(scheme), gspec, engine, vm.Config{
...
func TestTxIndexer(t *testing.T) {
	...
	_, blocks, receipts := GenerateChainWithGenesis(gspec, engine, 128, func(i int, gen *BlockGen) {
		tx, _ := types.SignTx(types.NewTransaction(nonce, common.HexToAddress("0xdeadbeef"), big.NewInt(1000), params.TxGas, big.NewInt(10*params.InitialBaseFee), nil), types.NewShanghaiSigner(common.Big1), testBankKey)
		gen.AddTx(tx)
		nonce += 1
	})
