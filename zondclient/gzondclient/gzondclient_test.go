...
func testCallContractWithBlockOverrides(t *testing.T, client *rpc.Client) {
	...
	if !bytes.Equal(res, common.FromHex("Z0000000000000000000000000000000000000000")) {
	...
	if !bytes.Equal(res, common.FromHex("Z1111111111111111111111111111111111111111")) {
		t.Fatalf("unexpected result: %x", res)
	}
}
