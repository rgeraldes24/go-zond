...
var (
	...
	testBankAddress common.Address = testBankKey.GetAddress()
...
func (b *testWorkerBackend) newRandomTx(creation bool) *types.Transaction {
	...
	if creation {
		tx, _ = types.SignTx(types.NewContractCreation(b.txPool.Nonce(testBankAddress), big.NewInt(0), testGas, gasPrice, common.FromHex(testCode)), types.NewShangaiSigner(common.Big1), testBankKey)
	} else {
		tx, _ = types.SignTx(types.NewTransaction(b.txPool.Nonce(testBankAddress), testUserAddress, big.NewInt(1000), params.TxGas, gasPrice, nil), types.NewShangaiSigner(common.Big1), testBankKey)
	}
	return tx
}
...
func TestEmptyWorkBeacon(t *testing.T) {
	testEmptyWork(t, beaconChainConfig, beacon.NewFaker())
}
...
func TestAdjustIntervalBeacon(t *testing.T) {
	testAdjustInterval(t, beaconChainConfig, beacon.NewFaker())
}
...
func TestGetSealingWorkBeacon(t *testing.T) {
	testGetSealingWork(t, beaconChainConfig, beacon.NewFaker())
}

func TestGetSealingWorkPostMerge(t *testing.T) {
	local := new(params.ChainConfig)
	*local = *beaconChainConfig
